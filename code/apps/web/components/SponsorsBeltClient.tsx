...
export default function SponsorsBeltClient({ sponsors }: { sponsors: Sponsor[] }) {
  ...
  const shouldMarquee = seq.length > 1;
  const displaySponsors = shouldMarquee ? [...seq, ...seq] : seq;

  const sponsorCard = (s: Sponsor & { _k: string }) => (
    <div
      className="
    ...
    >
      {s.logoUrl ? (
        ...
        <span className="text-xs text-neutral-500">{s.name || "Sponsor"}</span>
      )}
    </div>
  );
    ...
    <div className="relative mt-3 overflow-hidden">
      <div
        className={`inline-flex min-w-full items-center ${
          shouldMarquee
            ? "[animation:marquee_var(--speed)_linear_infinite] hover:[animation-play-state:paused]"
            : "justify-center"
        }`}
        style={
          {
      ...
      >
        {displaySponsors.map((s, i) =>
          s.url ? (
            ...
            <div key={`${s._k}-${i}`} aria-label={s.name || "Sponsor"}>
              {sponsorCard(s)}
            </div>
          )
        )}
